...
public class FirstPersonController : MonoBehaviour
{
    ...
    public float smoothTime = 0.05f; // smoothing time for camera
    #endregion
    ...
    private float velocitySmoothTime = 0.15f; // smoothing duration for movement
    private void Awake()
    {
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            float targetYaw = transform.localEulerAngles.y + Input.GetAxis("Mouse X") * mouseSensitivity;
            float mouseYInput = Input.GetAxis("Mouse Y") * mouseSensitivity;
            if (!invertCamera)
                currentPitch = Mathf.SmoothDamp(currentPitch, Mathf.Clamp(currentPitch - mouseYInput, -maxLookAngle, maxLookAngle), ref pitchVelocity, smoothTime);
            else
                currentPitch = Mathf.SmoothDamp(currentPitch, Mathf.Clamp(currentPitch + mouseYInput, -maxLookAngle, maxLookAngle), ref pitchVelocity, smoothTime);
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            // Calculate target velocity based on sprint or walk
            ...
            // Smoothly interpolate current velocity towards target velocity to reduce sudden changes
            Vector3 currentVelocity = new Vector3(rb.velocity.x, 0, rb.velocity.z);
            Vector3 velocityChange = Vector3.SmoothDamp(currentVelocity, targetVelocity, ref velocitySmooth, velocitySmoothTime);
            velocityChange.y = 0;

            Vector3 finalVelocityChange = velocityChange - currentVelocity;

            // Clamp velocity change for control and apply
            finalVelocityChange.x = Mathf.Clamp(finalVelocityChange.x, -maxVelocityChange, maxVelocityChange);
            finalVelocityChange.z = Mathf.Clamp(finalVelocityChange.z, -maxVelocityChange, maxVelocityChange);
            finalVelocityChange.y = 0;

            rb.AddForce(finalVelocityChange, ForceMode.VelocityChange);

            // Handle sprint state and sprint bar alpha for visual fade
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
            {
                isSprinting = (finalVelocityChange.x != 0 || finalVelocityChange.z != 0);
                if (isSprinting)
                {
                    if (isCrouched)
                    {
